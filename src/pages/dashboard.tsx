...
import { getAdminPanels } from 'apiSdk/admin-panels';
...
import { getTrafficGraphs } from 'apiSdk/traffic-graphs';
import { TrafficGraphInterface } from 'interfaces/traffic-graph';

import { AdminPanelListPage as ShowCaseList } from 'pages/admin-panels';

function DashboardPage() {
  const { session } = useSession();

  const adminPanelStat = useSWR<PaginatedInterface<AdminPanelInterface>>(
    () => `/admin-panels`,
    () => getAdminPanels({ limit: 1, offset: 0 }),
  );

  const deviceStat = useSWR<PaginatedInterface<DeviceInterface>>(
    () => `/devices`,
    () => getDevices({ limit: 1, offset: 0 }),
  );

  const trafficGraphStat = useSWR<PaginatedInterface<TrafficGraphInterface>>(
    () => `/traffic-graphs`,
    () => getTrafficGraphs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Admin Panels',
      result: adminPanelStat?.data?.totalCount,
      isLoading: adminPanelStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Devices',
      result: deviceStat?.data?.totalCount,
      isLoading: deviceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Traffic Graphs',
      result: trafficGraphStat?.data?.totalCount,
      isLoading: trafficGraphStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`admin-panels`} passHref legacyBehavior>
                  <Button
