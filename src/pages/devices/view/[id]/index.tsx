...
import { getDeviceById } from 'apiSdk/devices';
import { DeviceInterface } from 'interfaces/device';
import { AdminPanelListPage } from 'pages/admin-panels';
import { TrafficGraphListPage } from 'pages/traffic-graphs';

function DeviceViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<DeviceInterface>(
    () => (id ? `/devices/${id}` : null),
    () =>
      getDeviceById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('device', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/devices/edit/${id}`} passHref legacyBehavior>
                    <Button
