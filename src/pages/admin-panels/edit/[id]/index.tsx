...
import { getAdminPanelById, updateAdminPanelById } from 'apiSdk/admin-panels';
...
function AdminPanelEditPage() {
  ...
  const handleSubmit = async (values: AdminPanelInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAdminPanelById(id, values);
      mutate(updated);
      resetForm();
